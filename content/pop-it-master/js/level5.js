...
var level5 = new Phaser.Class({
    ...
    create: function () {
        ...
        level5popbase = this.add.image(5, 161, 'level5popbase').setOrigin(0, 0)
        var l2popxrr = [, 15, 33, 52, 78, 96, 114, 144, 161, 179, 178, 196, 234, 231, 268, 260, 327, 314, 300, 395, 385, 371, 58, 370, 70, 125, 180, 238, 295, 352, 80, 131, 183, 238, 291, 343, 91, 141, 192, 240, 286, 336, 106, 158, 208, 260, 313]
        var l2popyrr = [, 213, 256, 312, 215, 264, 324, 245, 294, 350, 174, 224, 294, 348, 178, 226, 219, 267, 330, 212, 255, 314, 367, 366, 415, 408, 406, 406, 408, 414, 467, 465, 461, 461, 461, 468, 517, 517, 517, 517, 517, 517, 572, 572, 572, 572, 572]
        for (i = 21; i >= 1; i--) {
            game['level5pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level5pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level5pop' + i].x += parseFloat(game['level5pop' + i].width / 2);
            game['level5pop' + i].y += parseFloat(game['level5pop' + i].height / 2);
        }
        for (i = 46; i >= 22; i--) {
            game['level5pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level5pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
        ...
        level5popupper = this.add.image(5, 161, 'level5popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level5popbase)
        for (i = 1; i <= 46; i++) {
            levelpop1.add(game['level5pop' + i])
        }
        levelpop1.add(level5popupper)
...
function level5start() {
    ...
    for (i = 46; i >= 1; i--) {
        game['level5pop' + i].on('pointerdown', level1popdown)
        game['level5pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level5pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level5pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 46)
                overlayscreen()
            }
