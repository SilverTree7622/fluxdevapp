...
var level14 = new Phaser.Class({
    ...
    create: function () {
        ...
        level14popbase = this.add.image(108, 101, 'level14popbase').setOrigin(0, 0)
        var l2popxrr = [, 128, 128, 128, 128, 128, 128, 181, 181, 181, 181, 181, 181, 230, 230, 230, 230, 230, 230, 230, 230, 283, 283, 283, 283, 283, 283, 283, 283]
        var l2popyrr = [, 379, 428, 478, 527, 578, 626, 379, 427, 477, 528, 578, 627, 276, 326, 376, 425, 475, 526, 575, 626, 278, 328, 376, 427, 476, 527, 576, 626]
        for (i = 28; i >= 1; i--) {
            game['level14pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level14pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level14pop' + i].x += parseFloat(game['level14pop' + i].width / 2);
            game['level14pop' + i].y += parseFloat(game['level14pop' + i].height / 2);
        }

        level14popupper = this.add.image(108, 248, 'level14popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level14popbase)
        for (i = 1; i <= 28; i++) {
            levelpop1.add(game['level14pop' + i])
        }
        levelpop1.add(level14popupper)
...
function level14start() {
    ...
    for (i = 28; i >= 1; i--) {
        game['level14pop' + i].on('pointerdown', level1popdown)
        game['level14pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level14pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level14pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 28)
                overlayscreen()
            }
