...
var level13 = new Phaser.Class({
    ...
    create: function () {
        ...
        level13popbase = this.add.image(0, 166, 'level13popbase').setOrigin(0, 0)
        var l2popxrr = [, 207, 206, 156, 206, 256, 116, 160, 208, 257, 305, 93, 139, 187, 234, 281, 329, 44, 91, 137, 186, 233, 282, 329, 376, 186, 233]
        var l2popyrr = [, 196, 247, 292, 292, 292, 355, 355, 355, 355, 355, 420, 420, 420, 420, 420, 420, 494, 494, 494, 494, 494, 494, 494, 494, 572, 572]
        for (i = 26; i >= 1; i--) {
            game['level13pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level13pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level13pop' + i].x += parseFloat(game['level13pop' + i].width / 2);
            game['level13pop' + i].y += parseFloat(game['level13pop' + i].height / 2);
        }

        level13popupper = this.add.image(0, 166, 'level13popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level13popbase)
        for (i = 1; i <= 26; i++) {
            levelpop1.add(game['level13pop' + i])
        }
        levelpop1.add(level13popupper)
...
function level13start() {
    ...
    for (i = 26; i >= 1; i--) {
        game['level13pop' + i].on('pointerdown', level1popdown)
        game['level13pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level13pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level13pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 26)
                overlayscreen()
            }
