...
var level4 = new Phaser.Class({
    ...
    create: function () {
        ...
        level4popbase = this.add.image(0, 170, 'level4popbase').setOrigin(0, 0)
        var l2popxrr = [, 147, 206, 266, 59, 117, 174, 232, 291, 349, 42, 102, 299, 361, 52, 110, 285, 343, 84, 143, 205, 264, 324, 121, 184, 244]
        var l2popyrr = [, 204, 204, 204, 267, 267, 267, 267, 267, 267, 338, 338, 338, 338, 407, 407, 407, 407, 475, 475, 475, 475, 475, 537, 537, 537]
        for (i = 3; i >= 1; i--) {
            game['level4pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level4pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level4pop' + i].x += parseFloat(game['level4pop' + i].width / 2);
            game['level4pop' + i].y += parseFloat(game['level4pop' + i].height / 2);
        }
        for (i = 9; i >= 4; i--) {
            game['level4pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level4pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
        ...
        level4popupper = this.add.image(0, 170, 'level4popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level4popbase)
        for (i = 1; i <= 25; i++) {
            levelpop1.add(game['level4pop' + i])
        }
        levelpop1.add(level4popupper)
...
function level4start() {
    ...
    for (i = 25; i >= 1; i--) {
        game['level4pop' + i].on('pointerdown', level1popdown)
        game['level4pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level4pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level4pop' + parseInt(sno)].setFrame(1)
                count = count + 1
