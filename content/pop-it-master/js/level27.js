...
var level27 = new Phaser.Class({
    ...
    create: function () {
        ...
        level27popbase = this.add.image(0, 212, 'level27popbase').setOrigin(0, 0)
        var l2popxrr = [, 14, 156, 254, 394, 41, 127, 177, 229, 281, 366, 122, 175, 229, 281, 127, 178, 230, 283, 154, 204, 258, 179, 233]
        var l2popyrr = [, 268, 232, 232, 269, 315, 298, 298, 298, 298, 314, 356, 356, 356, 356, 415, 415, 415, 415, 475, 475, 475, 533, 533]
        for (i = 23; i >= 1; i--) {
            game['level27pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level27pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level27pop' + i].x += parseFloat(game['level27pop' + i].width / 2);
            game['level27pop' + i].y += parseFloat(game['level27pop' + i].height / 2);
        }

        level27popupper = this.add.image(0, 212, 'level27popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level27popbase)
        for (i = 1; i <= 23; i++) {
            levelpop1.add(game['level27pop' + i])
        }
        levelpop1.add(level27popupper)
...
function level27start() {
    ...
    for (i = 23; i >= 1; i--) {
        game['level27pop' + i].on('pointerdown', level1popdown)
        game['level27pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level27pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level27pop' + parseInt(sno)].setFrame(1)
                count = count + 1
