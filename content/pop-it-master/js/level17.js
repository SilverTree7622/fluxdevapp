...
var level17 = new Phaser.Class({
    ...
    create: function () {
        ...
        level17popbase = this.add.image(0, 155, 'level17popbase').setOrigin(0, 0)
        var l2popxrr = [, 223, 68, 129, 193, 253, 318, 378, 70, 132, 192, 253, 313, 374, 3, 69, 129, 190, 251, 314, 70, 130, 192, 253, 312, 371, 69, 130, 316, 376]
        var l2popyrr = [, 164, 236, 236, 236, 236, 236, 236, 309, 309, 309, 309, 309, 309, 382, 382, 382, 382, 382, 382, 450, 450, 450, 450, 450, 450, 523, 523, 523, 523]
        for (i = 29; i >= 1; i--) {
            game['level17pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level17pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level17pop' + i].x += parseFloat(game['level17pop' + i].width / 2);
            game['level17pop' + i].y += parseFloat(game['level17pop' + i].height / 2);
        }

        level17popupper = this.add.image(0, 155, 'level17popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level17popbase)
        for (i = 1; i <= 29; i++) {
            levelpop1.add(game['level17pop' + i])
        }
        levelpop1.add(level17popupper)
...
function level17start() {
    ...
    for (i = 29; i >= 1; i--) {
        game['level17pop' + i].on('pointerdown', level1popdown)
        game['level17pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level17pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level17pop' + parseInt(sno)].setFrame(1)
                count = count + 1
