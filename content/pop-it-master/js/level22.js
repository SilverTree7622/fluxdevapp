...
var level22 = new Phaser.Class({
    ...
    create: function () {
        ...
        level22popbase = this.add.image(0, 168, 'level22popbase').setOrigin(0, 0)
        var l2popxrr = [, 174, 108, 155, 205, 260, 96, 145, 196, 249, 299, 62, 113, 164, 216, 268, 319, 34, 242, 290, 339, 229, 278, 327, 374, 205, 248, 296, 341, 382]
        var l2popyrr = [, 235, 289, 289, 289, 289, 341, 341, 341, 341, 341, 393, 393, 393, 393, 393, 391, 451, 445, 445, 445, 497, 497, 497, 497, 548, 548, 548, 548, 548]
        for (i = 29; i >= 1; i--) {
            game['level22pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level22pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level22pop' + i].x += parseFloat(game['level22pop' + i].width / 2);
            game['level22pop' + i].y += parseFloat(game['level22pop' + i].height / 2);
        }

        level22popupper = this.add.image(0, 168, 'level22popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level22popbase)
        for (i = 1; i <= 29; i++) {
            levelpop1.add(game['level22pop' + i])
        }
        levelpop1.add(level22popupper)
...
function level22start() {
    ...
    for (i = 29; i >= 1; i--) {
        game['level22pop' + i].on('pointerdown', level1popdown)
        game['level22pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level22pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level22pop' + parseInt(sno)].setFrame(1)
                count = count + 1
