...
var level11 = new Phaser.Class({
    ...
    create: function () {
        ...
        level11popbase = this.add.image(0, 196, 'level11popbase').setOrigin(0, 0)
        var l2popxrr = [, 70, 133, 194, 258, 320, 40, 100, 160, 223, 285, 349, 39, 101, 163, 226, 289, 351, 71, 133, 194, 256, 319, 109, 169, 229, 289]
        var l2popyrr = [, 251, 251, 251, 251, 251, 326, 326, 326, 326, 326, 326, 397, 397, 397, 397, 397, 397, 472, 472, 472, 472, 472, 543, 544, 544, 544]
        for (i = 26; i >= 1; i--) {
            game['level11pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level11pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level11pop' + i].x += parseFloat(game['level11pop' + i].width / 2);
            game['level11pop' + i].y += parseFloat(game['level11pop' + i].height / 2);
        }

        level11popupper = this.add.image(0, 196, 'level11popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level11popbase)
        for (i = 1; i <= 26; i++) {
            levelpop1.add(game['level11pop' + i])
        }
        levelpop1.add(level11popupper)
...
function level11start() {
    ...
    for (i = 26; i >= 1; i--) {
        game['level11pop' + i].on('pointerdown', level1popdown)
        game['level11pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level11pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level11pop' + parseInt(sno)].setFrame(1)
                count = count + 1
