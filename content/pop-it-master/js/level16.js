...
var level16 = new Phaser.Class({
    ...
    create: function () {
        ...
        level16popbase = this.add.image(40, 140, 'level16popbase').setOrigin(0, 0)
        var l2popxrr = [, 222, 274, 221, 274, 113, 166, 220, 276, 112, 166, 221, 276, 79, 137, 199, 259, 321, 56, 115, 169, 227, 282, 343, 124, 276]
        var l2popyrr = [, 185, 185, 243, 243, 309, 309, 309, 309, 370, 370, 370, 370, 429, 446, 454, 445, 428, 481, 514, 514, 514, 514, 483, 586, 586]
        for (i = 25; i >= 1; i--) {
            game['level16pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level16pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level16pop' + i].x += parseFloat(game['level16pop' + i].width / 2);
            game['level16pop' + i].y += parseFloat(game['level16pop' + i].height / 2);
        }

        level16popupper = this.add.image(40, 140, 'level16popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level16popbase)
        for (i = 1; i <= 25; i++) {
            levelpop1.add(game['level16pop' + i])
        }
        levelpop1.add(level16popupper)
...
function level16start() {
    ...
    for (i = 25; i >= 1; i--) {
        game['level16pop' + i].on('pointerdown', level1popdown)
        game['level16pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level16pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level16pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 25)
                overlayscreen()
            }
