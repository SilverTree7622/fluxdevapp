...
var level18 = new Phaser.Class({
    ...
    create: function () {
        ...
        level18popbase = this.add.image(7, 186, 'level18popbase').setOrigin(0, 0)
        var l2popxrr = [, 211, 20, 103, 211, 319, 402, 36, 387, 45, 92, 138, 188, 235, 283, 328, 372, 65, 113, 160, 206, 251, 299, 343, 63, 111, 158, 206, 255, 304, 350]
        var l2popyrr = [, 204, 252, 256, 256, 256, 250, 303, 301, 347, 335, 325, 322, 322, 327, 337, 348, 398, 387, 380, 380, 380, 385, 393, 453, 443, 439, 439, 439, 446, 455]
        for (i = 30; i >= 1; i--) {
            game['level18pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level18pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level18pop' + i].x += parseFloat(game['level18pop' + i].width / 2);
            game['level18pop' + i].y += parseFloat(game['level18pop' + i].height / 2);
        }

        level18popupper = this.add.image(7, 186, 'level18popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level18popbase)
        for (i = 1; i <= 30; i++) {
            levelpop1.add(game['level18pop' + i])
        }
        levelpop1.add(level18popupper)
...
function level18start() {
    ...
    for (i = 30; i >= 1; i--) {
        game['level18pop' + i].on('pointerdown', level1popdown)
        game['level18pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level18pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level18pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 30)
                overlayscreen()
            }
