...
var level7 = new Phaser.Class({
    ...
    create: function () {
        ...
        level7popbase = this.add.image(0, 185, 'level7popbase').setOrigin(0, 0)
        var l2popxrr = [, 60, 29, 19, 24, 44, 77, 125, 96, 86, 90, 108, 138, 154, 152, 162, 191, 233, 193, 196, 217, 253, 299, 258, 274, 302, 347, 347, 390, 137, 187, 252, 311]
        var l2popyrr = [, 301, 346, 406, 466, 527, 577, 289, 341, 402, 464, 522, 577, 399, 438, 485, 535, 578, 329, 392, 453, 506, 556, 347, 409, 465, 512, 412, 463, 194, 236, 263, 269]
        for (i = 32; i >= 1; i--) {
            game['level7pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level7pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level7pop' + i].x += parseFloat(game['level7pop' + i].width / 2);
            game['level7pop' + i].y += parseFloat(game['level7pop' + i].height / 2);
        }


        level7popupper = this.add.image(4, 176, 'level7popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level7popbase)
        for (i = 1; i <= 32; i++) {
            levelpop1.add(game['level7pop' + i])
        }
        levelpop1.add(level7popupper)
...
function level7start() {
    ...
    for (i = 32; i >= 1; i--) {
        game['level7pop' + i].on('pointerdown', level1popdown)
        game['level7pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level7pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level7pop' + parseInt(sno)].setFrame(1)
                count = count + 1
