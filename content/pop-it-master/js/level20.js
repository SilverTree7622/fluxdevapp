...
var level20 = new Phaser.Class({
    ...
    create: function () {
        ...
        level20popbase = this.add.image(0, 230, 'level20popbase').setOrigin(0, 0)
        var l2popxrr = [, 24, 40, 64, 97, 138, 184, 232, 278, 318, 350, 377, 393, 72, 92, 124, 165, 210, 254, 294, 319, 342, 128, 162, 206, 248, 283, 164, 205, 249, 8, 43, 85, 121, 158, 29, 65, 101, 137, 268, 305, 343, 384, 419, 289, 326, 361, 398]
        var l2popyrr = [, 386, 341, 298, 265, 244, 231, 231, 242, 267, 299, 339, 384, 378, 338, 306, 282, 279, 285, 305, 336, 376, 371, 339, 327, 339, 370, 399, 374, 396, 447, 440, 437, 439, 447, 475, 482, 482, 477, 449, 439, 438, 438, 446, 477, 482, 482, 475]
        for (i = 47; i >= 1; i--) {
            game['level20pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level20pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level20pop' + i].x += parseFloat(game['level20pop' + i].width / 2);
            game['level20pop' + i].y += parseFloat(game['level20pop' + i].height / 2);
        }

        level20popupper = this.add.image(0, 221, 'level20popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level20popbase)
        for (i = 1; i <= 47; i++) {
            levelpop1.add(game['level20pop' + i])
        }
        levelpop1.add(level20popupper)
...
function level20start() {
    ...
    for (i = 47; i >= 1; i--) {
        game['level20pop' + i].on('pointerdown', level1popdown)
        game['level20pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level20pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level20pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 47)
                overlayscreen()
            }
