...
var level3 = new Phaser.Class({
    ...
    create: function () {
        ...
        level3popbase = this.add.image(27, 133, 'level3popbase').setOrigin(0, 0)
        var l2popxrr = [, 183, 130, 200, 271, 64, 132, 200, 266, 332, 47, 109, 173, 234, 298, 361, 80, 141, 204, 267, 328, 108, 178, 246, 316, 136, 204, 270]
        var l2popyrr = [, 165, 234, 234, 234, 301, 301, 301, 301, 301, 368, 368, 368, 368, 368, 368, 436, 436, 436, 436, 436, 504, 504, 504, 504, 572, 572, 572]
        for (i = 1; i >= 1; i--) {
            game['level3pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level3pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level3pop' + i].x += parseFloat(game['level3pop' + i].width / 2);
            game['level3pop' + i].y += parseFloat(game['level3pop' + i].height / 2);
        }
        for (i = 4; i >= 2; i--) {
            game['level3pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level3pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
        ...
        level3popupper = this.add.image(27, 133, 'level3popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level3popbase)
        for (i = 1; i <= 27; i++) {
            levelpop1.add(game['level3pop' + i])
        }
        levelpop1.add(level3popupper)
...
function level3start() {
    ...
    for (i = 27; i >= 1; i--) {
        game['level3pop' + i].on('pointerdown', level1popdown)
        game['level3pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level3pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level3pop' + parseInt(sno)].setFrame(1)
                count = count + 1
