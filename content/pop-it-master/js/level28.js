...
var level28 = new Phaser.Class({
    ...
    create: function () {
        ...
        level28popbase = this.add.image(55, 143, 'level28popbase').setOrigin(0, 0)
        var l2popxrr = [, 81, 146, 241, 306, 144, 213, 172, 166, 135, 202, 139, 202, 266, 155, 219, 284]
        var l2popyrr = [, 186, 198, 222, 235, 263, 282, 332, 394, 457, 457, 521, 521, 521, 580, 580, 580]
        for (i = 16; i >= 1; i--) {
            game['level28pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level28pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level28pop' + i].x += parseFloat(game['level28pop' + i].width / 2);
            game['level28pop' + i].y += parseFloat(game['level28pop' + i].height / 2);
        }

        level28popupper = this.add.image(55, 143, 'level28popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level28popbase)
        for (i = 1; i <= 16; i++) {
            levelpop1.add(game['level28pop' + i])
        }
        levelpop1.add(level28popupper)
...
function level28start() {
    ...
    for (i = 16; i >= 1; i--) {
        game['level28pop' + i].on('pointerdown', level1popdown)
        game['level28pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level28pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level28pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 16)
                overlayscreen()
            }
