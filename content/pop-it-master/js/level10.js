...
var level10 = new Phaser.Class({
    ...
    create: function () {
        ...
        level10popbase = this.add.image(22, 128, 'level10popbase').setOrigin(0, 0)
        var l2popxrr = [, 320, 89, 145, 203, 260, 318, 49, 109, 168, 227, 287, 347, 79, 136, 195, 254, 313, 109, 170, 234, 297, 141, 200, 264, 170, 233]
        var l2popyrr = [, 162, 271, 271, 271, 271, 271, 336, 336, 336, 336, 336, 336, 401, 401, 401, 401, 401, 463, 463, 463, 463, 524, 524, 524, 590, 590]
        for (i = 26; i >= 1; i--) {
            game['level10pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level10pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level10pop' + i].x += parseFloat(game['level10pop' + i].width / 2);
            game['level10pop' + i].y += parseFloat(game['level10pop' + i].height / 2);
        }

        level10popupper = this.add.image(22, 124, 'level10popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level10popbase)
        for (i = 1; i <= 26; i++) {
            levelpop1.add(game['level10pop' + i])
        }
        levelpop1.add(level10popupper)
...
function level10start() {
    ...
    for (i = 26; i >= 1; i--) {
        game['level10pop' + i].on('pointerdown', level1popdown)
        game['level10pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level10pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level10pop' + parseInt(sno)].setFrame(1)
                count = count + 1
