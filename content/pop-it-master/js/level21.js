...
var level21 = new Phaser.Class({
    ...
    create: function () {
        ...
        level21popbase = this.add.image(72, 160, 'level21popbase').setOrigin(0, 0)
        var l2popxrr = [, 155, 206, 261, 101, 153, 203, 256, 310, 93, 144, 198, 248, 301, 118, 119, 119, 119, 172, 172, 172, 172, 222, 222, 222, 222, 276, 277, 277, 277]
        var l2popyrr = [, 201, 201, 201, 255, 255, 255, 255, 255, 311, 311, 311, 311, 311, 376, 427, 479, 530, 374, 428, 484, 535, 376, 427, 486, 543, 376, 431, 487, 541]
        for (i = 29; i >= 1; i--) {
            game['level21pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level21pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level21pop' + i].x += parseFloat(game['level21pop' + i].width / 2);
            game['level21pop' + i].y += parseFloat(game['level21pop' + i].height / 2);
        }

        level21popupper = this.add.image(72, 160, 'level21popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level21popbase)
        for (i = 1; i <= 29; i++) {
            levelpop1.add(game['level21pop' + i])
        }
        levelpop1.add(level21popupper)
...
function level21start() {
    ...
    for (i = 29; i >= 1; i--) {
        game['level21pop' + i].on('pointerdown', level1popdown)
        game['level21pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level21pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level21pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 29)
                overlayscreen()
            }
