...
var level2 = new Phaser.Class({
    ...
    create: function () {
        ...
        level2popbase = this.add.image(36, 116, 'level2popbase').setOrigin(0, 0)
        var l2popxrr = [, 115, 171, 228, 284, 63, 118, 173, 228, 283, 338, 91, 147, 201, 256, 311, 88, 142, 195, 252, 304, 113, 173, 233, 293, 113, 171, 229, 287, 143, 202, 260]
        var l2popyrr = [, 230, 230, 230, 230, 316, 315, 315, 315, 315, 315, 378, 378, 378, 378, 378, 440, 440, 440, 440, 440, 500, 500, 500, 500, 561, 561, 561, 561, 621, 621, 621]
        for (i = 10; i >= 1; i--) {
            game['level2pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level2pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level2pop' + i].x += parseFloat(game['level2pop' + i].width / 2);
            game['level2pop' + i].y += parseFloat(game['level2pop' + i].height / 2);
        }
        for (i = 15; i >= 11; i--) {
            game['level2pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level2pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
        ...
        level2popupper = this.add.image(36, 177, 'level2popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level2popbase)
        for (i = 1; i <= 31; i++) {
            levelpop1.add(game['level2pop' + i])
        }
        levelpop1.add(level2popupper)
...
function level2start() {
    ...
    for (i = 31; i >= 1; i--) {
        game['level2pop' + i].on('pointerdown', level1popdown)
        game['level2pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level2pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level2pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 31)
                overlayscreen()
            }
