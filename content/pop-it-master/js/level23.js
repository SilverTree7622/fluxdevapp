...
var level23 = new Phaser.Class({
    ...
    create: function () {
        ...
        level23popbase = this.add.image(0, 232, 'level23popbase').setOrigin(0, 0)
        var l2popxrr = [, 178, 226, 155, 203, 252, 155, 203, 250, 128, 178, 226, 276, 126, 176, 226, 277, 177, 227, 10, 65, 42, 396, 339, 363]
        var l2popyrr = [, 241, 239, 296, 296, 296, 346, 346, 346, 401, 401, 401, 401, 456, 456, 456, 456, 517, 517, 338, 356, 436, 337, 355, 435]
        for (i = 24; i >= 1; i--) {
            game['level23pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level23pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level23pop' + i].x += parseFloat(game['level23pop' + i].width / 2);
            game['level23pop' + i].y += parseFloat(game['level23pop' + i].height / 2);
        }

        level23popupper = this.add.image(0, 232, 'level23popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level23popbase)
        for (i = 1; i <= 24; i++) {
            levelpop1.add(game['level23pop' + i])
        }
        levelpop1.add(level23popupper)
...
function level23start() {
    ...
    for (i = 24; i >= 1; i--) {
        game['level23pop' + i].on('pointerdown', level1popdown)
        game['level23pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level23pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level23pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 24)
                overlayscreen()
            }
