...
var level19 = new Phaser.Class({
    ...
    create: function () {
        ...
        level19popbase = this.add.image(0, 191, 'level19popbase').setOrigin(0, 0)
        var l2popxrr = [, 94, 301, 21, 80, 140, 200, 259, 320, 380, 22, 81, 138, 200, 259, 319, 378, 49, 109, 170, 227, 287, 346, 103, 165, 223, 284, 199, 255, 304, 350]
        var l2popyrr = [, 209, 208, 276, 276, 276, 276, 276, 276, 276, 336, 336, 336, 337, 337, 337, 337, 402, 402, 402, 402, 402, 402, 460, 460, 460, 460, 530, 439, 446, 455]
        for (i = 27; i >= 1; i--) {
            game['level19pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level19pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level19pop' + i].x += parseFloat(game['level19pop' + i].width / 2);
            game['level19pop' + i].y += parseFloat(game['level19pop' + i].height / 2);
        }

        level19popupper = this.add.image(0, 191, 'level19popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level19popbase)
        for (i = 1; i <= 27; i++) {
            levelpop1.add(game['level19pop' + i])
        }
        levelpop1.add(level19popupper)
...
function level19start() {
    ...
    for (i = 27; i >= 1; i--) {
        game['level19pop' + i].on('pointerdown', level1popdown)
        game['level19pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level19pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level19pop' + parseInt(sno)].setFrame(1)
                count = count + 1
