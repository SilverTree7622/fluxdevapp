...
var level8 = new Phaser.Class({
    ...
    create: function () {
        ...
        level8popbase = this.add.image(0, 307, 'level8popbase').setOrigin(0, 0)
        var l2popxrr = [, 179, 233, 115, 141, 181, 227, 269, 297, 66, 84, 112, 156, 204, 253, 296, 328, 353, 12, 33, 65, 110, 159, 212, 264, 313, 354, 384, 406]
        var l2popyrr = [, 328, 328, 324, 364, 384, 387, 366, 330, 321, 367, 403, 426, 435, 428, 406, 373, 332, 322, 372, 417, 452, 475, 482, 476, 456, 423, 380, 337]
        for (i = 28; i >= 1; i--) {
            game['level8pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level8pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level8pop' + i].x += parseFloat(game['level8pop' + i].width / 2);
            game['level8pop' + i].y += parseFloat(game['level8pop' + i].height / 2);
        }

        level8popupper = this.add.image(0, 299, 'level8popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level8popbase)
        for (i = 1; i <= 28; i++) {
            levelpop1.add(game['level8pop' + i])
        }
        levelpop1.add(level8popupper)
...
function level8start() {
    ...
    for (i = 28; i >= 1; i--) {
        game['level8pop' + i].on('pointerdown', level1popdown)
        game['level8pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level8pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level8pop' + parseInt(sno)].setFrame(1)
                count = count + 1
