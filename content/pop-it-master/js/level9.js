...
var level9 = new Phaser.Class({
    ...
    create: function () {
        ...
        level9popbase = this.add.image(64, 118, 'level9popbase').setOrigin(0, 0)
        var l2popxrr = [, 155, 236, 111, 188, 268, 86, 156, 224, 294, 89, 158, 228, 294, 113, 184, 262, 161, 231]
        var l2popyrr = [, 225, 225, 298, 298, 298, 378, 378, 378, 378, 455, 455, 455, 455, 526, 526, 526, 605, 605]
        for (i = 18; i >= 1; i--) {
            game['level9pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level9pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level9pop' + i].x += parseFloat(game['level9pop' + i].width / 2);
            game['level9pop' + i].y += parseFloat(game['level9pop' + i].height / 2);
        }

        level9popupper = this.add.image(64, 201, 'level9popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level9popbase)
        for (i = 1; i <= 18; i++) {
            levelpop1.add(game['level9pop' + i])
        }
        levelpop1.add(level9popupper)
...
function level9start() {
    ...
    for (i = 18; i >= 1; i--) {
        game['level9pop' + i].on('pointerdown', level1popdown)
        game['level9pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level9pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level9pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 18)
                overlayscreen()
            }
