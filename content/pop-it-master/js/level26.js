...
var level26 = new Phaser.Class({
    ...
    create: function () {
        ...
        level26popbase = this.add.image(0, 259, 'level26popbase').setOrigin(0, 0)
        var l2popxrr = [, 5, 45, 377, 417, 39, 79, 118, 298, 338, 378, 56, 96, 135, 175, 248, 287, 328, 369, 94, 133, 172, 244, 283, 323, 90, 132, 172, 249, 289, 330, 115, 158, 267, 309]
        var l2popyrr = [, 276, 276, 276, 276, 324, 324, 324, 324, 324, 324, 370, 370, 370, 370, 370, 370, 370, 370, 416, 416, 416, 416, 416, 416, 463, 463, 463, 463, 463, 463, 511, 511, 511, 511]
        for (i = 34; i >= 1; i--) {
            game['level26pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level26pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level26pop' + i].x += parseFloat(game['level26pop' + i].width / 2);
            game['level26pop' + i].y += parseFloat(game['level26pop' + i].height / 2);
        }

        level26popupper = this.add.image(0, 259, 'level26popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level26popbase)
        for (i = 1; i <= 34; i++) {
            levelpop1.add(game['level26pop' + i])
        }
        levelpop1.add(level26popupper)
...
function level26start() {
    ...
    for (i = 34; i >= 1; i--) {
        game['level26pop' + i].on('pointerdown', level1popdown)
        game['level26pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level26pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level26pop' + parseInt(sno)].setFrame(1)
                count = count + 1
