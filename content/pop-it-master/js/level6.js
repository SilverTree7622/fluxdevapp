...
var level6 = new Phaser.Class({
    ...
    create: function () {
        ...
        level6popbase = this.add.image(42, 118, 'level6popbase').setOrigin(0, 0)
        var l2popxrr = [, 131, 197, 264, 96, 162, 229, 296, 62, 128, 198, 262, 329, 65, 128, 198, 262, 328, 97, 161, 230, 296, 128, 195, 261]
        var l2popyrr = [, 253, 253, 253, 322, 322, 322, 322, 390, 390, 390, 390, 390, 463, 463, 463, 463, 463, 531, 531, 531, 530, 601, 601, 601]
        for (i = 3; i >= 1; i--) {
            game['level6pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level6pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level6pop' + i].x += parseFloat(game['level6pop' + i].width / 2);
            game['level6pop' + i].y += parseFloat(game['level6pop' + i].height / 2);
        }
        for (i = 7; i >= 4; i--) {
            game['level6pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level6pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
        ...
        level6popupper = this.add.image(42, 225, 'level6popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level6popbase)
        for (i = 1; i <= 24; i++) {
            levelpop1.add(game['level6pop' + i])
        }
        levelpop1.add(level6popupper)
...
function level6start() {
    ...
    for (i = 24; i >= 1; i--) {
        game['level6pop' + i].on('pointerdown', level1popdown)
        game['level6pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level6pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level6pop' + parseInt(sno)].setFrame(1)
                count = count + 1
