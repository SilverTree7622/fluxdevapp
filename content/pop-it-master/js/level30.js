...
var level30 = new Phaser.Class({
    ...
    create: function () {
        ...
        level30popbase = this.add.image(0, 193, 'level30popbase').setOrigin(0, 0)
        var l2popxrr = [, 134, 200, 155, 225, 300, 44, 152, 220, 292, 359, 44, 140, 214, 284, 350, 30, 156, 226, 296, 141, 211]
        var l2popyrr = [, 215, 215, 283, 283, 283, 355, 353, 353, 353, 353, 413, 413, 413, 413, 413, 478, 478, 478, 478, 544, 544]
        for (i = 21; i >= 1; i--) {
            game['level30pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level30pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level30pop' + i].x += parseFloat(game['level30pop' + i].width / 2);
            game['level30pop' + i].y += parseFloat(game['level30pop' + i].height / 2);
        }

        level30popupper = this.add.image(0, 193, 'level30popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level30popbase)
        for (i = 1; i <= 21; i++) {
            levelpop1.add(game['level30pop' + i])
        }
        levelpop1.add(level30popupper)
...
function level30start() {
    ...
    for (i = 21; i >= 1; i--) {
        game['level30pop' + i].on('pointerdown', level1popdown)
        game['level30pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level30pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level30pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 21)
                overlayscreen()
            }
