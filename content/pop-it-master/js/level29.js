...
var level29 = new Phaser.Class({
    ...
    create: function () {
        ...
        level29popbase = this.add.image(0, 206, 'level29popbase').setOrigin(0, 0)
        var l2popxrr = [, 175, 228, 57, 108, 168, 228, 286, 346, 59, 118, 172, 230, 288, 347, 6, 63, 121, 177, 232, 287, 345, 400, 36, 94, 147, 202, 259, 315, 371, 92, 145, 202, 256, 310]
        var l2popyrr = [, 220, 220, 276, 276, 276, 276, 276, 276, 333, 333, 333, 333, 333, 333, 389, 389, 389, 389, 389, 389, 389, 389, 444, 444, 444, 444, 444, 444, 444, 497, 497, 497, 497, 497]
        for (i = 34; i >= 1; i--) {
            game['level29pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level29pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level29pop' + i].x += parseFloat(game['level29pop' + i].width / 2);
            game['level29pop' + i].y += parseFloat(game['level29pop' + i].height / 2);
        }

        level29popupper = this.add.image(0, 206, 'level29popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level29popbase)
        for (i = 1; i <= 34; i++) {
            levelpop1.add(game['level29pop' + i])
        }
        levelpop1.add(level29popupper)
...
function level29start() {
    ...
    for (i = 34; i >= 1; i--) {
        game['level29pop' + i].on('pointerdown', level1popdown)
        game['level29pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level29pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level29pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 34)
                overlayscreen()
            }
