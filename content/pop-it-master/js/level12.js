...
var level12 = new Phaser.Class({
    ...
    create: function () {
        ...
        level12popbase = this.add.image(0, 137, 'level12popbase').setOrigin(0, 0)
        var l2popxrr = [, 98, 152, 205, 258, 312, 46, 99, 152, 205, 258, 310, 364, 46, 97, 153, 205, 255, 310, 361, 70, 122, 177, 230, 282, 335, 99, 150, 203, 256, 306, 152, 204, 257]
        var l2popyrr = [, 247, 247, 247, 247, 247, 314, 314, 314, 314, 314, 314, 314, 385, 385, 385, 385, 385, 385, 385, 454, 454, 454, 454, 454, 454, 519, 519, 519, 519, 519, 586, 586, 586]
        for (i = 33; i >= 1; i--) {
            game['level12pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level12pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level12pop' + i].x += parseFloat(game['level12pop' + i].width / 2);
            game['level12pop' + i].y += parseFloat(game['level12pop' + i].height / 2);
        }

        level12popupper = this.add.image(0, 213, 'level12popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level12popbase)
        for (i = 1; i <= 33; i++) {
            levelpop1.add(game['level12pop' + i])
        }
        levelpop1.add(level12popupper)
...
function level12start() {
    ...
    for (i = 33; i >= 1; i--) {
        game['level12pop' + i].on('pointerdown', level1popdown)
        game['level12pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level12pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level12pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 33)
                overlayscreen()
            }
