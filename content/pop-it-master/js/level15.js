...
var level15 = new Phaser.Class({
    ...
    create: function () {
        ...
        level15popbase = this.add.image(36, 121, 'level15popbase').setOrigin(0, 0)
        var l2popxrr = [, 94, 354, 105, 123, 123, 123, 123, 123, 123, 58, 317, 306, 290, 290, 290, 290, 290, 290, 181, 179, 181, 181, 181, 181, 181, 235, 235, 235, 235, 235, 235, 235]
        var l2popyrr = [, 144, 194, 269, 328, 385, 441, 499, 558, 615, 194, 147, 271, 329, 385, 443, 500, 557, 615, 209, 329, 386, 443, 500, 557, 614, 207, 329, 386, 443, 501, 558, 615]
        for (i = 32; i >= 1; i--) {
            game['level15pop' + i] = this.add.sprite(l2popxrr[i], l2popyrr[i], 'level15pop' + i).setOrigin(0.5, 0.5).setInteractive({
                pixelPerfect: true,
                useHandCursor: true
            });
            game['level15pop' + i].x += parseFloat(game['level15pop' + i].width / 2);
            game['level15pop' + i].y += parseFloat(game['level15pop' + i].height / 2);
        }

        level15popupper = this.add.image(36, 121, 'level15popupper').setOrigin(0, 0)

        levelpop1 = this.add.container()
        levelpop1.add(level15popbase)
        for (i = 1; i <= 32; i++) {
            levelpop1.add(game['level15pop' + i])
        }
        levelpop1.add(level15popupper)
...
function level15start() {
    ...
    for (i = 32; i >= 1; i--) {
        game['level15pop' + i].on('pointerdown', level1popdown)
        game['level15pop' + i].on('pointermove', level1popdown)
    }
    ...
    function level1popdown(pointer) {
        if (pointer.isDown) {
            ...
            if (game['level15pop' + parseInt(sno)].frame.name == 0) {
                playsoundeffects('clickss')
                game['level15pop' + parseInt(sno)].setFrame(1)
                count = count + 1

                level1lpbar.x = level1lpbar.x + (271 / 32)
                overlayscreen()
            }
