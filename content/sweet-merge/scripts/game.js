class Game extends Phaser.Scene {
	...
	create(){
		...
		function gameover(){
			...
			let bRestart = createButton(540, 1020, 'restart', self);
			let bHome = createButton(540, 1180, 'home', self);
			const scoreTxt = self.add.text(716, 693, score.toString(), {fontFamily: 'vanilla', fontSize: 50, align: 'right',color: '#ffffff'}).setOrigin(1, 0.5);
			const bestScoreTxt = self.add.text(716, 873, bestscore.toString(), {fontFamily: 'vanilla', fontSize: 50, align: 'right',color: '#ffffff'}).setOrigin(1, 0.5);

			const rewardBtn = createButton(
				540, 1340, 'reward', self
			).setDisplaySize(bRestart.width, bRestart.height);
			if (!self.hasUsedReward && UtilPlatform.chckIsApp()) {
			// if (!self.hasUsedReward) {
				const loading = self.add.sprite(0, 0, 'loading').setVisible(false).setScale(2);
				loading.setDepth(1);
				rewardBtn.on('pointerup', async () => {
					...
					rewardBtn.setTint(0x8c8c8c).disableInteractive();
					bRestart.setTint(0x8c8c8c).disableInteractive();
					bHome.setTint(0x8c8c8c).disableInteractive();
					...
					UtilPlatform.sendMsg2Parent(
						'ShowRewardVideo',
						{
							...
							dismiss: (isRewarded) => {
								...
								state = 'play';
								[ dark, bgPopup, title, bRestart, bHome, scoreTxt, bestScoreTxt, loading, rewardBtn, ].forEach(obj => {
									obj.setVisible(false).destroy();
								});
								// 가장 위에 있는 오브젝트 10개 제거
								const objects = self.matter.world.localWorld.bodies
									.filter(body => body.gameObject && body.gameObject.isObject)
									.sort((a, b) => a.position.y - b.position.y)
									.slice(0, 10)
								;
								objects.forEach(body => {
									if (body.gameObject) {
										addPoolObject(body.gameObject);
									}
								});
								self.matter.world.resume();
								return;
