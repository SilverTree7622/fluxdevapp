...
var randomArrx = [0];
var randomArry = [0];
var level30 = new Phaser.Class({
    Extends: Phaser.Scene,
    initialize: function level30() {
    ...
    preload: function () {
        ...
        randomArrx = [0]
        randomArry = [0]
        timearr = [1, 0, 0, 0, 0, 0, 0, 0]
        timevalue = 1
        time = timevalue * 60
    },
    create: function () {

        level30background = this.add.image(0, 0, 'level1background').setOrigin(0, 0).setInteractive()
        level30wood = this.add.image(64, 37, 'level1wood').setOrigin(0, 0)
        level30blast = this.add.sprite(150, 209, 'level1blast').setOrigin(0, 0)

        level30blast.visible = false
        ...
        for (var i = 50; i >= 1; i--) {
            ...
            do {
                ...
                y = Phaser.Math.Between(180, 765);
            } while (checkCollision(x, y, randomArrx, randomArry));

            randomArrx.push(x);
            randomArry.push(y);

            game['level30nail' + i] = this.add.sprite(x, y, 'level1nail' + i).setOrigin(0.5, 0.5).setInteractive();
            game['level30nail' + i].setFrame(1);
            game['level30nail' + i].visible = false
        }
        game['level30nail' + 1].visible = true
        ...
        levelpop1.add(level30wood)
        levelpop1.add(bomb)
        levelpop1.add(explode)
        for (i = 50; i >= 1; i--) {
            levelpop1.add(game['level30nail' + i])
        }

        levelpop1.add(level30blast)
        ...
        level30blast.anims.load('level1blast')
        level30blast.x = 354
        level30blast.y = 422
        anim = game.scene.scenes[pageNo].anims.create({
            key: 'bomb',
        ...
        function timecheck() {
            ...
            if (timearr[0] > 0 && timearr[1] >= 0 && nailcount <= 50 && nextnail) {
            ...
            } else if (timearr[0] == 0 && timearr[1] > 0 && nailcount <= 50 && nextnail) {
                timearr[1] = timearr[1] - 1
                if (timearr[1] > 9) {
                    sec.setText(timearr[1])
                } else {
        ...
        level30lpbg = this.add.image(135, 93, 'level1lpbg').setOrigin(0, 0)
        level30lpbar4 = this.add.image(-148, 93, 'level1lpmask4').setOrigin(0, 0)
        level30lpbar3 = this.add.image(-148, 93, 'level1lpmask3').setOrigin(0, 0)
        level30lpbar2 = this.add.image(-148, 93, 'level1lpmask2').setOrigin(0, 0)
        level30lpbar1 = this.add.image(-148, 93, 'level1lpmask1').setOrigin(0, 0)
        level30lpmask1 = this.add.image(135, 93, 'level1lpmask1').setOrigin(0, 0).setVisible(false);
        mask = level30lpmask1.createBitmapMask();
        mask.alpha = 0
        
        level30lpbar4.setMask(mask);
        level30lpbar3.setMask(mask);
        level30lpbar2.setMask(mask);
        level30lpbar1.setMask(mask);
        emojis = this.add.sprite(122, 85, 'emoji').setOrigin(0, 0).setScale(0.7)

        level30stresstxt = this.add.image(180, 49, 'level1stresstxt').setOrigin(0, 0)



        for (i = 50; i >= 1; i--) {
            game['level30nail' + i].on('pointerdown', hammerFun)
        }
        ...
        function hammerFun() {
            ...
            if (game['level30nail' + narr1[0]].x >= 75 && game['level30nail' + narr1[0]].x <= 215) {
                lhammer.x = game['level30nail' + narr1[0]].x - 65
                lhammer.y = game['level30nail' + narr1[0]].y - 45
                lhammer.anims.play('lhammer')
                lhammer.visible = true
            } else if (game['level30nail' + narr1[0]].x >= 215 && game['level30nail' + narr1[0]].x <= 354) {
                shammer.x = game['level30nail' + narr1[0]].x - 45
                shammer.y = game['level30nail' + narr1[0]].y - 45
                shammer.anims.play('shammer')
                shammer.visible = true
            } else if (game['level30nail' + narr1[0]].x >= 355 && game['level30nail' + narr1[0]].x <= 495) {
                rhammer.x = game['level30nail' + narr1[0]].x - 45
                rhammer.y = game['level30nail' + narr1[0]].y - 45
            ...
            if (game['level30nail' + narr1[0]].frame.name == 1) {
                game['level30nail' + narr1[0]].setFrame(harr1[1])
                setTimeout(hiddenHammer, 200)
            } else if (game['level30nail' + narr1[0]].frame.name == 2) {
                game['level30nail' + narr1[0]].setFrame(harr1[2])
                setTimeout(hiddenHammer, 200)
            } else if (game['level30nail' + narr1[0]].frame.name == 3) {
                game['level30nail' + narr1[0]].setFrame(harr1[3])
                game['level30nail' + narr1[0]].disableInteractive()
                game.scene.scenes[pageNo].tweens.add({
                    targets: game['level30nail' + narr1[0]],
                    alpha: 0,
                    ease: 'Back',
                    delay: 300,
                    duration: 300,
                ...
                if (narr1[0] <= 50) {
                    ...
                    if (nailcount < 50) {
                        game['level30nail' + (nailcount + 1)].visible = true
                    }
                }
        ...
        function newnail() {
            ...
            shammer.visible = false
            lhammer.visible = false
            level30lpbar1.x = level30lpbar1.x + (282 / 50)
            level30lpbar2.x = level30lpbar2.x + (282 / 50)
            level30lpbar3.x = level30lpbar3.x + (282 / 50)
            level30lpbar4.x = level30lpbar4.x + (282 / 50)
            if (nailcount == 50) {
                nextnail = false
        ...
        function bombFun() {
            ...
            level30blast.visible = true
            bomb.visible = false
            level30blast.x = bomb.x
            level30blast.y = bomb.y
            level30blast.anims.play('level1blast')

            for (i = 50; i >= 1; i--) {
                game['level30nail' + i].disableInteractive()
            }
        ...
        bomb.alpha = 0
        if (nailcount < 50 && nextnail && timearr[1] != 0) {
      ...
      failedpanel = this.add.image(240, 427, 'failedpanel').setOrigin(0.5, 0.5)
        ...
        level30home = this.add.sprite(450, 37, 'level1home').setOrigin(0.5, 0.5).setInteractive()
        level30home.setScale(0.85)
        level30home.on('pointerdown', homeFun)
        level30home.on('pointerover', homeoverFun)
        level30home.on('pointerout', homeoutFun)
    ...
    update: function () {
        ...
        if ((Math.round(level30lpbar1.x) <= 134)) {
            if (Math.round(level30lpbar1.x) <= -77.5) {
                emojis.setFrame(0)
                level30lpbar1.setVisible(true);
                level30lpbar2.setVisible(false);
                level30lpbar3.setVisible(false);
                level30lpbar4.setVisible(false);
                emojis.x = Math.round(level30lpbar1.x) + 252
            } else if ((Math.round(level30lpbar1.x) > -77.5) && (Math.round(level30lpbar1.x) <= -7)) {
                emojis.setFrame(1)
                level30lpbar1.setVisible(false);
                level30lpbar2.setVisible(true);
                level30lpbar3.setVisible(false);
                level30lpbar4.setVisible(false);
                emojis.x = Math.round(level30lpbar1.x) + 252
            } else if ((Math.round(level30lpbar1.x) > -7) && (Math.round(level30lpbar1.x) <= 63.5)) {
                emojis.setFrame(2)
                level30lpbar1.setVisible(false);
                level30lpbar2.setVisible(false);
                level30lpbar3.setVisible(true);
                level30lpbar4.setVisible(false);
                emojis.x = Math.round(level30lpbar1.x) + 252
            } else if ((Math.round(level30lpbar1.x) > 63.5) && (Math.round(level30lpbar1.x) <= 134)) {
                emojis.setFrame(3)
                level30lpbar1.setVisible(false);
                level30lpbar2.setVisible(false);
                level30lpbar3.setVisible(false);
                level30lpbar4.setVisible(true);
                emojis.x = Math.round(level30lpbar1.x) + 252
            }
        } else if ((Math.round(level30lpbar1.x) > 134)) {
            level30lpbar1.x = 134
            level30lpbar2.x = 134
            level30lpbar3.x = 134
            level30lpbar4.x = 134
