...
var level2 = new Phaser.Class({
    ...
    create: function () {
        level2background = this.add.image(0, 0, 'level1background').setOrigin(0, 0).setInteractive()
        level2wood = this.add.image(64, 37, 'level1wood').setOrigin(0, 0)
        level2blast = this.add.sprite(150, 209, 'level1blast').setOrigin(0, 0)
        level2blast.visible = false
        for (i = 4; i >= 1; i--) {
            randomArrx.push(Phaser.Math.Between(100, 455));
            randomArry.push(Phaser.Math.Between(220, 765));
        }
        for (i = 4; i >= 1; i--) {
            game['level2nail' + i] = this.add.sprite(randomArrx[i] + 20, randomArry[i], 'level1nail' + i).setOrigin(0.5, 0.5).setInteractive()
            game['level2nail' + i].setFrame(1)
            game['level2nail' + i].visible = false
        }
        game['level2nail' + 1].visible = true
        ...
        levelpop1.add(level2wood)
        for (i = 4; i >= 1; i--) {
            levelpop1.add(game['level2nail' + i])
        }
        levelpop1.add(level2blast)
        ...
        level2blast.anims.load('level1blast')
        level2blast.x = 354
        level2blast.y = 422
        level2lpbg = this.add.image(135, 93, 'level1lpbg').setOrigin(0, 0)
        level2lpbar4 = this.add.image(-148, 93, 'level1lpmask4').setOrigin(0, 0)
        level2lpbar3 = this.add.image(-148, 93, 'level1lpmask3').setOrigin(0, 0)
        level2lpbar2 = this.add.image(-148, 93, 'level1lpmask2').setOrigin(0, 0)
        level2lpbar1 = this.add.image(-148, 93, 'level1lpmask1').setOrigin(0, 0)
        level2lpmask1 = this.add.image(135, 93, 'level1lpmask1').setOrigin(0, 0).setVisible(false);
        mask = level2lpmask1.createBitmapMask();
        mask.alpha = 0
        level2lpbar4.setMask(mask);
        level2lpbar3.setMask(mask);
        level2lpbar2.setMask(mask);
        level2lpbar1.setMask(mask);
        emojis = this.add.sprite(122, 85, 'emoji').setOrigin(0, 0).setScale(0.7)
        level2stresstxt = this.add.image(180, 49, 'level1stresstxt').setOrigin(0, 0)
        timecheck()
        function timecheck() {
            if (timearr[0] > 0 && timearr[1] >= 0 && nailcount <= 5 && nextnail) {
            ...
            } else if (timearr[0] == 0 && timearr[1] > 0 && nailcount <= 5 && nextnail) {
                timearr[1] = timearr[1] - 1
                if (timearr[1] > 9) {
                    sec.setText(timearr[1])
                } else {
        ...
        for (i = 4; i >= 1; i--) {
            game['level2nail' + i].on('pointerdown', hammerFun)
        }
        function hammerFun() {
            ...
            if (game['level2nail' + narr1[0]].frame.name == 1) {
                game['level2nail' + narr1[0]].setFrame(harr1[1])
                setTimeout(hiddenHammer, 200)
            } else if (game['level2nail' + narr1[0]].frame.name == 2) {
                game['level2nail' + narr1[0]].setFrame(harr1[2])
                setTimeout(hiddenHammer, 200)
            } else if (game['level2nail' + narr1[0]].frame.name == 3) {
                game['level2nail' + narr1[0]].setFrame(harr1[3])
                game['level2nail' + narr1[0]].disableInteractive()
                setTimeout(hiddenHammer, 200)
                game.scene.scenes[pageNo].tweens.add({
                    targets: game['level2nail' + narr1[0]],
                    alpha: 0,
                    ease: 'Back',
                    delay: 500,
                    duration: 300,
                ...
                if (narr1[0] <= 4) {
                    ...
                    if (nailcount < 4) {
                        game['level2nail' + (nailcount + 1)].visible = true
                    }
                }
            }
        }
        function newnail() {
            ...
            shammer.visible = false
            lhammer.visible = false
            level2lpbar1.x = level2lpbar1.x + (282 / 4)
            level2lpbar2.x = level2lpbar2.x + (282 / 4)
            level2lpbar3.x = level2lpbar3.x + (282 / 4)
            level2lpbar4.x = level2lpbar4.x + (282 / 4)
            if (nailcount == 4) {
                nextnail = false
            }
        }
        ...
        level2home = this.add.sprite(450, 37, 'level1home').setOrigin(0.5, 0.5).setInteractive()
        level2home.setScale(0.85)
        level2home.on('pointerdown', homeFun)
        level2home.on('pointerover', homeoverFun)
        level2home.on('pointerout', homeoutFun)
        function homeoutFun() {
            this.setScale(0.85)
        }
    ...
    update: function () {
        if ((Math.round(level2lpbar1.x) <= 134)) {
            if (Math.round(level2lpbar1.x) <= -77.5) {
                emojis.setFrame(0)
                level2lpbar1.setVisible(true);
                level2lpbar2.setVisible(false);
                level2lpbar3.setVisible(false);
                level2lpbar4.setVisible(false);
                emojis.x = Math.round(level2lpbar1.x) + 252
            } else if ((Math.round(level2lpbar1.x) > -77.5) && (Math.round(level2lpbar1.x) <= -7)) {
                emojis.setFrame(1)
                level2lpbar1.setVisible(false);
                level2lpbar2.setVisible(true);
                level2lpbar3.setVisible(false);
                level2lpbar4.setVisible(false);
                emojis.x = Math.round(level2lpbar1.x) + 252
            } else if ((Math.round(level2lpbar1.x) > -7) && (Math.round(level2lpbar1.x) <= 63.5)) {
                emojis.setFrame(2)
                level2lpbar1.setVisible(false);
                level2lpbar2.setVisible(false);
                level2lpbar3.setVisible(true);
                level2lpbar4.setVisible(false);
                emojis.x = Math.round(level2lpbar1.x) + 252
            } else if ((Math.round(level2lpbar1.x) > 63.5) && (Math.round(level2lpbar1.x) <= 134)) {
                emojis.setFrame(3)
                level2lpbar1.setVisible(false);
                level2lpbar2.setVisible(false);
                level2lpbar3.setVisible(false);
                level2lpbar4.setVisible(true);
                emojis.x = Math.round(level2lpbar1.x) + 252
            }
        } else if ((Math.round(level2lpbar1.x) > 134)) {
            level2lpbar1.x = 134
            level2lpbar2.x = 134
            level2lpbar3.x = 134
            level2lpbar4.x = 134
