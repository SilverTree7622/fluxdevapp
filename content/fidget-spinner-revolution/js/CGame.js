function CGame(){
    ...
    this._init = function(){   
        ...
        var iPriceFriction;
        if(_iCurFrictionIndex === PRICE_FRICTION.length-1){
            iPriceFriction = -1;
        }else{
            iPriceFriction = PRICE_FRICTION[_iCurFrictionIndex+1];
        }
                 
        var iPriceSpeed;
        if(_iCurMaxSpeedIndex === PRICE_MAX_SPEED.length-1){
            iPriceSpeed = -1;
        }else{
            iPriceSpeed = PRICE_MAX_SPEED[_iCurMaxSpeedIndex+1];
        }

        
        _oInterface = new CInterface(iPriceFriction,iPriceSpeed);   
